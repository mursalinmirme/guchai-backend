export function buildSystemPrompt(context: { today: string; timezone?: string; userEmail?: string; memories?: any[] }) {
...
Today's date is ${context.today}.${context.timezone ? ` The user's timezone is ${context.timezone}.` : ""} ${context.userEmail ? `The user's email is ${context.userEmail}.` : ""}${memoryStr}
